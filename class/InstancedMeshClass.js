...
export default class InstancedMeshClass {
    ...
    gsapAnimations() {
        const from = {
            rotation: { y: 0, x: 0 },
            position: { y: this.layer.y, x: this.layer.distance }
          };
          const to = {
            rotation: { x: this.layer.to.rotation.x, y: 180 },
            position: { y: this.layer.to.position.y, x: this.layer.to.position.x }
          };
          ...
          const tl = gsap.timeline({
            ...
            repeatDelay: 2,
            onUpdate: () => {
              for (let i = 0; i < this.layer.quantity; i++) {
                const radianY = THREE.MathUtils.degToRad(this.offsets[i]);
                const radianX = THREE.MathUtils.degToRad(dummy.rotation.x);
          
                dummy.rotation.y = radianY;
                dummy.rotation.x = radianX*-1;
          
                dummy.position.y = from.position.y;
                dummy.position.z = Math.cos(radianY) * from.position.x * -1;
                dummy.position.x = Math.sin(radianY) * from.position.x * -1;
